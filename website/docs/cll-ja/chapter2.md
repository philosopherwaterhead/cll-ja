...
例 1.1 のような 静的な関係 を記述するときには、 名詞 ＜父親＞を用いる。  
例 1.2 のような 動的な関係 を記述するときには、 動詞 ＜殴る＞を用いる。  
例 1.3 のような 連体（限定）的な関係 を記述する時には、 形容詞 ＜高い＞を用いる。  
...
3例とも＜与える＞によって表現される文の関係は同一である。（最後の例は、意味的には変だが）  
...
このブリディは例 5.2 の形に対応している。つまり、以下の形式である。   
...
例 5.7 のように`zo'e`がブリディの最後にない場合は省略できない。  
...
例 6.1 では、セルブリの前にスムティ(x1)はひとつしかない。  
...
次の例では、`tavla` のPSであるが, ２種類の解釈が可能である。
```
...
例 12.4 では、他の内容を示すときに１つのスムティが使われている。  
...
英語と同じように、例 15.8 は次のように言い換えることができる。  
...
17.3)  la djan. [cu] klama le zarci
      ジョンは店に 行く（行った、行くだろう）
```
というように、、例 17.1, 17.2 のいずれにも当てはまる。  
