...
ロジバンは適切な発話を一意に記述できるうえ、適切な記述も一意に発話できるよう設計されている。  
そのため、ロジバンの標準的な表記は、音素に対して固有の文字か記号を割り当てなければならない。  
それぞれの文字や記号は、1つの音、より正確に言うと、その音素の発音に許された範囲の音しか持たない。  
また、ロジバンの単語を認識するために必要なアクセント（音声の強調）や休止（言葉の間）を示す記号もある。  
他の言語では、句読点や声調で表現されるものが、ロジバンでは全て文字で表される。  
それら2つの性質を合わせて、専門的な用語で "_audio-visual isomorphism_" と呼ぶ。  
  
ロジバンは変形されたラテン（ローマ）アルファベットを使用し、以下の文字と記号で構成される。  
`' , . a b c d e f g i j k l m n o p r s t u v x y z`  
`h`、`q`、`w` の文字は省かれている。  

上記のアルファベットは、コンピュータで広く使用されているASCII文字コードと同じ順序である。  
ロジバンのアルファベット順序をASCIIと同じにすることで、コンピュータによる並べ替えや検索が容易になる。  

アルファベットの大文字は、ロジバン化した名前の、アクセントがある音節を表すときのみ使用される。  
したがって、通常発音される英語名の "Josephine" は、ロジバン化されて "DJOsefin." となり、発音は ['dʒosɛfinʔ] となる。  
（角括弧内の記号の説明は3章2節を参照。）  

### 3.2 基本的な音声
  
ロジバンの発音は、国際音声記号（International Phonetic Alphabet, IPA）という、発音記号の標準的な転写方法で定義されている。  
例えば、"cat" という単語は、一般的なアメリカ人は、[kæt] と発音する。  
3章10節では、本章で使用するIPA文字について、英語で最も近い類義語と共に説明している。  
この説明は、音声を解説するための専門用語に慣れない者にとって、特に有用である。  
ロジバンの標準的な発音と、許容される異表記を下の表に示す。  
母国語や方言による発音の違いをカバーするため、あえて少し曖昧な表記にしている。  
...
ロジバンは、勘違いされない様に、はっきり発音する必要がある。  
正しく発音するには発声と舌の位置が重要だが、ロジバンのコミュニティでは、子音間の微妙な差異が生じる。  
発音に関する必須規則は現時点でこれだけである。  
  
特に、ロジバンの母音は唇を丸めて発音することも、丸めずに発音することもできる。  
通常、`o` と `u` は唇を丸め、それ以外は丸めないが、`y` を丸める人もいる（必須条件ではない）。  
ロジバンの子音は、有気音と無気音にすることができる。  
ロシア語などに見られる子音の口蓋化は、標準的な発音では認められないが、後続の `i` が原因で発生する場合がある。 
   
文字の `c`、`g`、`j`、`s`、および、`x` で表される音は、英語の正書では曖昧な音（`c`、`g`、`s`）であるため、英語圏の人々にとって特に注意が必要である。  
というのも、ロジバンでは（`c`、`j`、`x`）であり、音が著しく異なるからである。  
...
子音ではないが、一般的な2つの英語音がロジバンで見られる。"church" の "ch" と、"judge" の "j" である。  
ロジバンでは、それらの音を母音抜きで話す子音とみなすため、tc (IPA [tʃ]) と dj (IPA [dʒ]) という、異なる2つの子音で表現する。  
一般的に、複合音を1音とするか、2音とするかは言語によって異なる。  
...
アポストロフィ、ピリオド、コンマには特に注意が必要である。  
これらは全て音節区切り`(# division between syllables)`の指標として使われる。   
しかし、それぞれは異なる発音と理由で使われる。

アポストロフィ`「'」`は英語の`h (IPA [h])`に近い音素を表す。しかし、文字 `h` はこの音を表すのには使わない。これには理由が2つある。  
まず、形態論の説明を簡潔にするため、それから、音がかなりありふれたものであるため、すっきりした見た目で表現するためである。   
アポストロフィの音は、本来は子音だが、ロジバンでは4章で説明する形態論の目的から、子音や母音として扱わない。    
また、見た目が似ているコンマやピリオドと同様、アポストロフィも（用途は異なるが）音節の分離に使われる。  

アポストロフィは、母音同士を1つの単語内で繋げる一方で、それぞれの滑らかな分離を可能にする。   
実際に、アポストロフィを無声の母音わたり音 `（# glide）` とみなすこともできる。

ロジバンで使われない無声摩擦音は、何でもアポストロフィの異音として許容される。  
`[h]` の代替音を使用する際は、聞き手の利便が最優先されるべきである。

ピリオド「.」は義務的な小休止を表すが、長さに決まりはない。声門破裂音（IPA [ʔ]）は最も短い長さの小休止とみなす。   
小休止（あるいは声門破裂音）は任意の単語間に現れてよいが、特定の場合（4章9節で詳述）は必須である。  
特に、母音で始まる単語は、直前に必ずピリオドを置き、子音で終わる単語では、直後に必ずピリオドを置く。  
専門的な話をすると、義務的な小休止を置くべき位置の法則は、曖昧でないため、ある位置にピリオドがなくとも、  
他の正しい文章から推察できる。そのため、ピリオドはもっぱら義務的な小休止の読み手の備忘録であり、読む手助けをするものである。

一見、単語内にピリオドがあるように見えることがある。  
その場合、その文字列は1単語ではなく2単語である。`（# ピリオドが文字列を2単語に分割する）`  
ピリオドがあるなら、必ずしも単語間に空白を置く必要はない。

コンマ `「,」` は、単語内の音節区切りを示すのに使われ、通常は音節の区切り方が、聞き手にとって明白でない場合に用いられる。  
コンマは音節を区切るが、ピリオドと対照的に、そこに小休止が（絶対に）ないことを示す。  
2つの母音の間のコンマは、（そこで単語を区切ってしまう）小休止、ピリオドを避けるために、ある種のわたり音が必要な可能性を示す。  
その場合、コンマの発音に、アポストロフィの音（IPA `[h]`）を使えば安全である。   
一方、ピリオドの音（小休止や声門閉鎖音）は単語を区切ってしまうため、コンマの発音に使用できない。

それ以外の場合、コンマは通常、音節子音 `l`, `m`, `n`, `r` の存在を明確化するために使用される（後で説明する）。  
この場合、コンマは本質的に不要である：コンマの有無によってロジバンの単語が異なることはないため。
...
ロジバンには16の二重母音が存在する。二重母音は、短い母音と半母音の2つで構成される母音である。  
半母音は両唇半母音 (IPA [w]) または硬口蓋半母音 (IPA [j]) であり、母音に先行（開始半母音）または後続（終了半母音）する。  
二重母音は常に単一の音節で構成される。  
  
ロジバンでは、母音とは音節の核となる、比較的長い音声のことを指す。  
子音は比較的短い音のため、聞き取るには通常、母音を伴う必要がある。  
子音は音節の先頭か末尾、母音の周囲に発生することがあり、位置に関わらず、複数の子音がまとめて現れる可能性がある。  
母音はそれぞれ個別の音節を構成し、子音はそれに対して影響を及ぼさない。  
  
ロジバンの6つの母音は、`a`、`e`、`i`、`o`、`u` そして、`y` である。  
最初の5つの母音は、ロジバンのあらゆる単語に対し、自由に使用される。  
母音 `y` の使用は限定的で、ロジバン化した名前、アルファベットで表したロジバン名、  
複合語を接着する用途としての母音、空白を埋める単語として（英語の "uh" や "er"に相当）単独で使われる。
...
副強調を規定する規則はほとんど無く、通常は話し手の母語習慣や好みに従う。副強調は、文脈やある概念の強調に使用することが出来る。  
